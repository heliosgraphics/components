...
const ThemeSwitcher: React.FC<{}> = () => {
	...
	useEffect(() => {
		const currentTheme: FUIThemes | null = localStorage?.getItem("theme") as FUIThemes;
		const prefersDarkScheme = globalThis?.matchMedia("(prefers-color-scheme: dark)").matches;

		const newTheme: FUIThemes = !!currentTheme ? currentTheme : prefersDarkScheme ? "dark" : "light";
	...
	return <Toggle onChange={toggleTheme} label="" isChecked={dark} id="theme-switcher" />;
};

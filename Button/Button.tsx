...
const Button: React.FC<ButtonProps> = (props) => {
	...
	const buttonClasses = classNames(styles.button, "relative sans cursor-pointer", {
		// Types
	...
	const buttonLoadingSize: 10 | 20 = props.size && props.size !== "normal" ? 10 : 20;
	const buttonInputType: "submit" | "button" = props.type === "primary" && !props.color ? "submit" : "button";

	// const onButtonClick = (event: React.MouseEvent): void => props.onClick?.(event);

	return (
		<Flex className={buttonClasses} isInline={true} isCentered={true} onClick={props.onClick} data-component="Button">
			{!!props.icon && !isIconOnlyLoading && (
				<Flex className={styles.button__icon}>
					<Icon name={props.icon} size={BUTTON_ICON_SIZE[props.size || "normal"]} />
				</Flex>
			)}
			{!!props.flair && (
			...
			/>
			{props.isLoading && <Loading size={buttonLoadingSize} color="currentcolor" className={styles.button__loading} />}
		</Flex>
	);
};

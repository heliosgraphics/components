...
const Dropdown: React.FC<DropdownProps> = ({ isHidden, children, items, isDisabled, position = "right" }) => {
	...
	const mouseEnter = (_event: React.MouseEvent<HTMLDivElement>): void => setHovering(true);
	const mouseLeave = (_event: React.MouseEvent<HTMLDivElement>) => {
		setHovering(false);

		return setTimeout(() => !stateRef.current && setVisible(false), LOADING_FAST_DELAY);
	};
